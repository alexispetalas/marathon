...
class MarathonSchedulerActor private (
  ...
  def started: Receive = LoggingReceive.withLabel("started") {
    ...
    case DeploymentManagerActor.CancelDeployment(plan) =>
      deploymentManager.cancel(plan).onComplete{
        case Success(d) => self ! DeploymentFinished(plan)
  ...
  def deploy(origSender: ActorRef, cmd: Deploy): Unit = {
    ...
    // DEBUG: ---DON'T FORGET TO REMOVE LOGGING----
    val resFuture = deploymentManager.start(plan, cmd.force, origSender)
    resFuture.onComplete{
      case Success(d) =>
        logger.info(s">>>>> SUCCESSFULLy deployed $plan")
        self ! DeploymentManagerActor.DeploymentFinished(plan)
      case Failure(t) =>
        logger.error(s">>>>> FAILED to deploy $plan because: ", t)
        self ! DeploymentManagerActor.DeploymentFailed(plan, t)
    }
    // DEBUG: ---END
  }
